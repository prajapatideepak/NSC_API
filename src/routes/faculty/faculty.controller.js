...
async function getFacultydetails(req, res) {
  try {
    ...
    one_staff_Details = await staffs
...
async function editFaculty(req, res) {
  try {
    ...
    const basic_info_id = await BasicInfo.findByIdAndUpdate(
      staff_details.basic_info_id,
      {
    ...
    const contact_info_id = await ContactInfo.findByIdAndUpdate(
      staff_details.contact_info_id,
      {
